...
**Type:** scoped tooling contribution on a **fork**. **Not** an FM effort.
**Fork:** `m9h/SpikeLab` (origin), `upstream`=[braingeneers/SpikeLab](https://github.com/braingeneers/SpikeLab) v0.1.2.
**Already set up:** clone at **`/mnt/t9/dev/SpikeLab`**, branch **`openscope-adapter`**, git identity
Morgan Hough <morgan.hough@gmail.com>, remotes origin=m9h upstream=braingeneers.
**Prereq met:** feasibility gate passed 2026-06-23 (SpikeLab loads Allen OpenScope ecephys NWB →
`SpikeData` cleanly; see "Verified facts").
...
**EDA skills** (deviant-vs-standard PSTH, pool QC sweep). **Commit everything to the fork branch
`openscope-adapter` and push to `origin` (m9h/SpikeLab).** Keep the *generic* unit-locations fix
as its own separate commit(s) so it can become an upstream PR later — but **do NOT open a PR to
braingeneers/SpikeLab.** Reuse the already-validated parsing in `openscope_nwb.py` — do **not**
re-derive it.

## Verified facts (don't re-discover)
- **Install (no CUDA):** `uv pip install "spikelab[neo,io] @ git+https://github.com/braingeneers/SpikeLab.git"` (Python 3.12). `[neo]`=pynwb, `[io]`=pandas. **Avoid** `[kilosort4]`,`[hippie]` (Torch/CUDA → GB10 sm_121 wall). A ready venv already exists: `/mnt/t9/venvs/spikelab-eval` (install your editable fork into it: `VIRTUAL_ENV=/mnt/t9/venvs/spikelab-eval uv pip install -e /mnt/t9/dev/SpikeLab`).
...
- **Eval artifacts on T9 (reuse):** sample session `/mnt/t9/tmp/spikelab-eval/sub-647836_ses-1227858756_ogen.nwb` (2.15 GB, already downloaded).
...
(copy/vendor it into the adapter) provides, pure-numpy, the OpenScope NWB drift handling the gate
proved is needed — **validated across 6 dandisets**:
...
## Deliverables (ordered) — all committed to `openscope-adapter`, pushed to `origin`
1. **`spikelab_openscope` adapter** (new module in the fork, separate from SpikeLab core):
...
2. **EDA skills** (follow SpikeLab's own `src/spikelab/agent/` skill structure):
   ...
   `pool_qc_sweep` (one session per READY dandiset, run `curate_by_*` + report).
3. **Generic fix — separate commit(s):** make `load_spikedata_from_nwb` populate
   `unit_locations`/`neuron_attributes` from `electrodes_by_channel` for Allen-style NWB (currently
   returns `None`). Keep GENERIC (not OpenScope-specific) and in their own commits so an upstream PR
   is trivial later. Optional follow-up commit: `remfile`/fsspec streaming. **Do NOT open the PR.**
...
- **Commit to the `m9h/SpikeLab` fork only.** Do **NOT** open a PR to braingeneers/SpikeLab without
  explicit go-ahead. Keep generic fixes as separate commits so a PR is easy later.
- **Don't touch** the `spinning-up-in-openscope` repo except to *read*/copy `openscope_nwb.py`.

## Constraints
- **Home NVMe is ~98% full** — everything on **`/mnt/t9`** (1.2 TB free). Fork clone is at
  `/mnt/t9/dev/SpikeLab`; venv `/mnt/t9/venvs/spikelab-eval`; data `/mnt/t9/tmp/spikelab-eval/`.
  If the global uv cache errors, `export UV_CACHE_DIR=/mnt/t9/uv-cache-mhough`.
- Loader is **path-based** → download sessions to T9 (smallest 000253 `_ogen` ≈ 2.1 GB) unless/until
  the streaming commit lands.
- Commit author is preset (Morgan Hough). End commit messages with the Co-Authored-By / Claude
...
- The `load_spikedata_from_nwb` unit-locations fix is committed as separate generic commit(s).
- 001637/001191 handled gracefully (region/none, no crash), logged note that they need CCF reg.
- Branch `openscope-adapter` pushed to `origin` (m9h/SpikeLab). **No upstream PR.**

## Suggested first actions
Read this brief and `openscope_nwb.py`; install the fork editable into the eval venv; reproduce the
gate (`load_spikedata_from_nwb` on the cached `sub-647836` session) before building. At the end,
push `openscope-adapter` to `origin` and report the branch URL + adapter module path. Do not open an
upstream PR.
